...
//                    checkselected = true;
        ...
        for (product sanPham : productList) {
            if (sanPham.isStatus()) {
                addItem(sanPham);
//                addItem(new product(sanPham.get(), sanPham.getTenSP(), 
//                        sanPham.getHinh(), sanPham.getGia(), 
//                        sanPham.getGhiChu(), sanPham.getLoaiSP(), 
//                        sanPham.getKhuyenMai(), sanPham.isTrangThai()));
            }
        }
    }
    ...
    public void fillToTable(product sanPham) {
        ...
        try {
            ...
            listMaSP.add(String.valueOf(sanPham.getProductId()));
            listGia.add(giakm);
//            listKM.add(sanPham.getKhuyenMai());
    ...
    public boolean checkSP(product item) {
        String giaFD2 = "";
        for (int i = 0; i < listMaSP.size(); i++) {
            int ma = Integer.parseInt(listMaSP.get(i));
            if (item.getProductId() == ma) {
                if (item.getPrice() != listGia.get(listGia.size() - 1)) {
...
//        }
    ...
    public void SearchProduct() {
        productList = pdao.getProductbyName(txtSearch.getText());
